...
export function isSqlSafe(sql: string) {
  const s = sql.toLowerCase();
  if (s.includes('insert') || s.includes('update') || s.includes('delete') || s.includes('drop') || s.includes('alter') || s.includes('truncate')) {
    return false;
  }
  ...
  const mentionsAllowed = allowed.some(t => s.includes(t));
